    ...
    try {
      ...
      if ((isByteDanceCdn || isDolaReferrer) && isVideoFile) {
        ...
        if (cleanUrl && (cleanUrl.startsWith('http://') || cleanUrl.startsWith('https://'))) {
          console.log(`[Dola Downloader] Safety Net: Intercepted external raw HTTP download #${downloadItem.id} for "${cleanUrl}". Cancelling to clean watermark.`);

          // Cancel and erase external raw download so the watermarked file does not land on disk
          chrome.downloads.cancel(downloadItem.id, () => {
            chrome.downloads.erase({ id: downloadItem.id }, () => {});
          });

          // Route into watermark removal queue with force=true so in-browser cleaner processes it into <subfolder>/cleaned/
  ...
  if (message.type === 'TRIGGER_PAGE_SCAN_AND_DOWNLOAD') {
    (async () => {
      try {
        ...
        // Re-inject extractor and content script dynamically to ensure latest hooks are active
        await chrome.scripting.executeScript({
