  ...
  chrome.downloads.onCreated.addListener(async (downloadItem) => {
    try {
      ...
      if ((isByteDanceCdn || isDolaReferrer) && isVideoFile) {
        ...
        // Why: Only intercept and cancel external browser downloads if we have a valid, fetchable HTTP(S) stream URL.
        // If the URL is a local blob: or non-HTTP stream generated by the page, cancelling it would destroy
        // the user's download without a fallback. Allowing non-HTTP downloads guarantees the user never loses a file.
        if (cleanUrl && (cleanUrl.startsWith('http://') || cleanUrl.startsWith('https://'))) {
          const canonicalKey = dolaExtractCanonicalKey(cleanUrl);
          if (dolaDownloadedKeys.has(canonicalKey) || dolaInProgressKeys.has(canonicalKey)) {
            console.log(`[Dola Downloader] Safety Net: Cancelling redundant external download #${downloadItem.id} (already tracked).`);
          ...
          console.log(`[Dola Downloader] Safety Net: Intercepted external HTTP download #${downloadItem.id} for "${cleanUrl}". Redirecting to master pipeline.`);
          ...
          // If the URL is a local blob: or non-HTTP stream generated by the page,
          // do NOT cancel it. Allow it to download normally so the user never loses their file.
          console.log(`[Dola Downloader] Safety Net: Preserving external non-HTTP/blob download #${downloadItem.id} ("${cleanUrl}").`);
        }
      }
    } catch (err) {
...
async function dolaHandleAutoDownload(video, force = false) {
  ...
  // Why: Studio Relay architecture — streams originating from fallback_api are already
  // 100% pristine unwatermarked 1080p master MP4 files requested directly from ByteDance CDN
  // with logo_type: 'unwatermarked'. They must NEVER be routed into canvas inpainting.
  const isFallbackMaster = video.source === 'fallback_api' || video.isRawMaster === true || video.isUnwatermarked === true || video.watermarkType === 'none';
  const isExplicitDynamic = !isFallbackMaster && Boolean(video.watermarkType === 'dynamic' || cleanUrl.includes('video_gen_watermark_dyn'));
  const isExplicitStatic = !isFallbackMaster && !isExplicitDynamic && Boolean(video.watermarkType === 'static' || cleanUrl.includes('video_gen_watermark'));
  const needsWatermarkCleaning = !isFallbackMaster && (isExplicitDynamic || isExplicitStatic);
  const watermarkType = isFallbackMaster ? 'none' : (isExplicitDynamic ? 'dynamic' : (isExplicitStatic ? 'static' : 'none'));
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'TRIGGER_PAGE_SCAN_AND_DOWNLOAD') {
    (async () => {
      try {
        let activeTab = null;
        // Check active tab in current window first to target what the user is actively viewing
        const [currentTab] = await chrome.tabs.query({ active: true, currentWindow: true });
        if (currentTab && currentTab.url && (currentTab.url.includes('dola.com') || currentTab.url.includes('doubao.com'))) {
          activeTab = currentTab;
        } else {
          // Broaden query to include apex domains and all subdomains
          const dolaTabs = await chrome.tabs.query({
            url: [
              'https://*.dola.com/*',
              'https://dola.com/*',
              'https://*.doubao.com/*',
        ...
        // Send message directly to preserve in-memory state in extractor and content script.
        // Only inject scripts dynamically as an emergency fallback if the tab was loaded before extension start.
        let scanRes = null;
        try {
          scanRes = await chrome.tabs.sendMessage(activeTab.id, { type: 'SCAN_AND_DOWNLOAD_ACTIVE_TAB' });
        } catch (msgErr) {
          if (msgErr?.message?.includes('Receiving end does not exist')) {
            console.log('[Dola Downloader] Content script not detected, injecting dynamically...');
            await chrome.scripting.executeScript({
            ...
            scanRes = await chrome.tabs.sendMessage(activeTab.id, { type: 'SCAN_AND_DOWNLOAD_ACTIVE_TAB' }).catch(() => null);
          } else {
            throw msgErr;
          }
        }
        ...
        if (scanRes?.ok && scanRes.foundCount > 0) {
          sendResponse({ ok: true, downloadedCount: scanRes.foundCount, message: scanRes.message || `Downloaded ${scanRes.foundCount} master video(s)` });
        } else {
