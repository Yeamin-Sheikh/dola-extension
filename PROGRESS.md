...
#### What changed
- Fixed manual download failure when auto-download is turned off: resolved issue where clicking native download buttons in Dola AI or Doubao or clicking Download on Screen failed to download or remove watermarks.
- Added React Fiber inspection in `extractor.js`: inspects `__reactProps$` and walks `__reactFiber$` component trees up to 15 levels on any clicked button, video element, or chat message container to retrieve underlying ByteDance CDN video streams and prompts when DOM elements only expose local `blob:` URLs.
- Added live DOM and Fiber scanner `scanDomForVideosLive()` in `extractor.js` triggered by `DOLA_GET_CHAT_MEDIA` and download clicks.
- Broadened network request interceptors in `extractor.js`: captures video signatures across all `/api/`, `/conversation/`, `/im/`, and `/video/` endpoints instead of restricting to two fixed URLs.
- Updated click interceptor `handlePageDownloadClick` in `content.js`: removed unconditional `preventDefault()` and `stopImmediatePropagation()`. Native event propagation is preserved if video streams are not resolved synchronously, preventing dead or frozen download buttons.
- Updated `triggerDownload` in `content.js`: keys are only recorded in `downloadedMediaKeys` when the background service worker confirms the item is queued or started.
- Updated safety net interceptor in `background.js`: guarded `chrome.downloads.onCreated` to ensure external downloads are only cancelled when a valid HTTP(S) stream URL is resolved. Local `blob:` URLs generated by the page are preserved to prevent file loss.
- Improved tab matching in `background.js` for `TRIGGER_PAGE_SCAN_AND_DOWNLOAD` and `TRIGGER_SHOW_VIDEOS_IN_CHAT`: prioritizes the active tab in the current window and covers apex domains (`https://dola.com/*`, `https://doubao.com/*`).
- Added robust error handling and null checks in `popup.js` for `btnDownloadScreen`.
- Bumped extension version to `v2.4.1` across `manifest.json`, `sidepanel.html`, and `README.md`.
- Re-packaged `dola-extension.zip`.

#### Files touched
- `extractor.js`: Added `findVideoInReactFiber`, `scanDomForVideosLive`, `DOLA_PAGE_DOWNLOAD_CLICKED` dispatcher, and broadened API response inspection.
- `content.js`: Added safe click handling in `handlePageDownloadClick`, deferred deduplication in `triggerDownload`, and `DOLA_PAGE_DOWNLOAD_CLICKED` listener.
- `background.js`: Guarded `chrome.downloads.onCreated` against non-HTTP erasure, and prioritized active tab with apex domain matching.
- `popup.js`: Hardened manual download button state and null-safety.
- `manifest.json`: Bumped version to 2.4.1.
- `sidepanel.html`: Bumped version badges to v2.4.1.
- `README.md`: Bumped version badges to v2.4.1.
- `PROGRESS.md`: Documented Session 31.
